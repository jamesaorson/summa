...
SUMMA_ARRAY_GENERATE_TYPE_DEF(SummaHashKeys, hash_keys, SummaHashCode)
...
SUMMA_ARRAY_GENERATE_TYPE_IMPL(SummaHashKeys, hash_keys, SummaHashCode)
...
void summa_hash_set_clear(SummaHashSet set) {
    summa_hash_keys_clear(set->keys);
}

void summa_hash_set_copy(SummaHashSet dest, SummaHashSet src) {
    dest->element_size = src->element_size;
    if (dest->keys) {
        summa_hash_keys_free(dest->keys);
        dest->keys = summa_hash_keys_make_empty();
    }
    if (dest->values) {
        summa_hash_values_free(dest->values);
        dest->values = summa_hash_values_make_empty();
    }
    summa_hash_keys_copy(dest->keys, src->keys);
    summa_hash_values_copy(dest->values, src->values);
}

void summa_hash_set_free(SummaHashSet set) {
    summa_hash_values_free(set->values);
    set->values = nullptr;
    summa_hash_keys_free(set->keys);
    set->keys = nullptr;
    free(set);
}
